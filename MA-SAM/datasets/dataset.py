...
def random_rot_flip(image, label, uncertainty):
    k = np.random.randint(0, 4)
    image = np.rot90(image, k, axes=(0, 1))
    label = np.rot90(label, k, axes=(0, 1))
    uncertainty = np.rot90(uncertainty, k, axes=(0, 1))
    axis = np.random.randint(0, 2)
    image = np.flip(image, axis=axis).copy()
    label = np.flip(label, axis=axis).copy()
    uncertainty = np.flip(uncertainty, axis=axis).copy()
    return image, label, uncertainty


def random_rotate(image, label, uncertainty):
    angle = np.random.randint(-15, 15)
    image = ndimage.rotate(image, angle, order=0, reshape=False)
    label = ndimage.rotate(label, angle, order=0, reshape=False)
    uncertainty = ndimage.rotate(uncertainty, angle, order=0, reshape=False)
    return image, label, uncertainty
...
def random_erasing(
    ...
    uncertainty[sy : sy + ny, sx : sx + nx] = 0.0


    return imgs, label, uncertainty
...
class RandomGenerator(object):
    ...
    def __call__(self, sample):
        image, label, uncertainty = sample["image"], sample["label"], sample["uncertainty"]
        # print(image.shape)

        if random.random() > 0.5:
            image, label, uncertainty = random_rot_flip(image, label, uncertainty)
        if random.random() > 0.5:
            image, label, uncertainty = random_rotate(image, label, uncertainty)
        if random.random() > 0.5:
            image, label = adjust_light(image, label)
        if random.random() > 0.5:
            image, label, uncertainty = random_erasing(imgs=image, label=label, uncertainty=uncertainty, rng=self.rng)
        ...
        low_res_label = zoom(
            label, (self.low_res[0] / label_h, self.low_res[1] / label_w, 1.0), order=0
        )

        uncertainty = average_pooling(uncertainty)
...
class dataset_reader(Dataset):
    def __init__(self, csv_path, num_classes, transform=None):
    ...
    def __getitem__(self, idx):
        data = read_image(self.sample_list[idx]) # [256, 256, 5] if 2D_all_5slice else [1, 256, 256]
        ...
        uncertainty = read_image(self.sample_list[idx].replace(".pkl", "_uncertainty_10.pkl"))
        ...
        # label = label.reshape(label.shape[1], label.shape[2], label.shape[0])

        sample = {"image": image, "label": label, "uncertainty": uncertainty}
        if self.transform:
