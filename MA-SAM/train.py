...
from sam_fact_tt_image_encoder import Fact_tt_Sam, Fact_tt_un_Sam
...
parser.add_argument(
    "--csv_path",
    type=str,
    help="path csv for training data",
)
parser.add_argument(
    "--output",
    type=str,
...
parser.add_argument(
    "--base_lr", type=float, default=0.0008, help="segmentation network learning rate"
)

parser.add_argument(
    "--max_epochs", type=int, default=1000, help="maximum epoch number to train"
)
parser.add_argument(
    "--stop_epoch", type=int, default=1000, help="maximum epoch number to train"
)
...
parser.add_argument(
    "--AdamW", action="store_true", help="If activated, use AdamW to finetune SAM model"
)
parser.add_argument("--module", type=str, default="sam_fact_tt_image_encoder")
parser.add_argument("--dice_param", type=float, default=0.8)
parser.add_argument(
    "--lr_exp", type=float, default=7, help="The learning rate decay expotential"
)
...
parser.add_argument(
    "--tf32",
    action="store_true",
    help="If activated, use tf32 to accelerate the training process",
)
parser.add_argument(
    "--compile",
    action="store_true",
    help="If activated, compile the training model for acceleration",
)
parser.add_argument(
    "--use_amp",
    action="store_true",
    help="If activated, adopt mixed precision for acceleration",
)
parser.add_argument(
    "--skip_hard",
    action="store_true",
