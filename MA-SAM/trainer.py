...
def trainer_run(args, model, snapshot_path, multimask_output, low_res):
    ...
    print("Starting to create DataLoader...")
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            image_batch, label_batch, uncertainty_batch = sampled_batch["image"], sampled_batch["label"], sampled_batch["uncertainty"] # [B, n, H, W]
            image_batch = image_batch.unsqueeze(2) # [B, n, 1, H, W]
            image_batch = torch.cat((image_batch, image_batch, image_batch), dim=2) # [B, n, 3, H, W]
            hw_size = image_batch.shape[-1]
            label_batch = label_batch.contiguous().view(-1, hw_size, hw_size)

            low_res_label_batch = sampled_batch["low_res_label"]
            image_batch, label_batch, uncertainty_batch = image_batch.cuda(), label_batch.cuda(), uncertainty_batch.cuda()
            ...
            if args.use_amp:
                with torch.autocast(
                    ...
                    if args.finetune_method == "MA_UN_SAM":
                        outputs = model(image_batch, multimask_output, args.img_size, uncertainty_batch)
                    else:
            ...
            if args.use_amp:
                with torch.autocast(
                    ...
                    if args.finetune_method == "MA_UN_SAM":
                        outputs = model(image_batch, multimask_output, args.img_size, None)
                    else:
                        outputs = model(image_batch, multimask_output, args.img_size)
            ...
            logging.info(
                "iteration %d : loss : %f, loss_ce: %f, loss_dice: %f"
                % (iter_num, loss1.item(), loss_ce1.item(), loss_dice1.item())
            )
